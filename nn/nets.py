...
class AlexNet(Net):
    ...
    def __init__(self, in_channels, out_channels, momentum=0, nesterov=False):
        ...
        self.conv1 = Conv2d(in_channels, 227, 227, 96, stride=4, padding=0, momentum=momentum, nesterov=nesterov)
        self.conv2 = Conv2d(96, 5, 5, 256, stride=1, padding=2, momentum=momentum, nesterov=nesterov)
        self.conv3 = Conv2d(256, 3, 3, 384, stride=1, padding=1, momentum=momentum, nesterov=nesterov)
        self.conv4 = Conv2d(384, 3, 3, 384, stride=1, padding=1, momentum=momentum, nesterov=nesterov)
        self.conv5 = Conv2d(384, 3, 3, 256, stride=1, padding=1, momentum=momentum, nesterov=nesterov)

        self.maxPool1 = MaxPool(3, 3, 96, stride=2)
        self.maxPool2 = MaxPool(3, 3, 256, stride=2)
        self.maxPool3 = MaxPool(3, 3, 256, stride=2)
        self.fc1 = FC(9216, 4096, momentum=momentum, nesterov=nesterov)
        self.fc2 = FC(4096, 4096, momentum=momentum, nesterov=nesterov)
        self.fc3 = FC(4096, out_channels, momentum=momentum, nesterov=nesterov)
    ...
    def forward(self, inputs):
        ...
        self.U1 = F.dropout(x.shape, 0.5)
        x *= self.U1
        x = self.relu7(self.fc2(x))
        self.U2 = F.dropout(x.shape, 0.5)
        x *= self.U2
