...
def train():
    train_loader, test_loader = pynet.vision.data.load_cifar10_pytorch(data_path, batch_size=batch_size, shuffle=True)

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    net = pytorch.models.nin(in_channels=3).to(device)
